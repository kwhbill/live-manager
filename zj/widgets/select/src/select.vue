<template>
  <div
    ...
    <transition
      name="el-zoom-in-top"
      @before-enter="handleMenuEnvisibleter"
    ...
    watch: {
      ...
      query(val) {
        ...
        if (this.remote && typeof this.remoteMethod === 'function') {
          ...
          this.broadcast('ZjTree', 'resetIndex');
        } else if (typeof this.filterMethod === 'function') {
          this.filterMethod(val);
          this.broadcast('ElOptionGroup', 'queryChange');
        } else {
          this.filteredOptionsCount = this.optionsCount;
          this.broadcast('ZjTree', 'queryChange', val);
          this.broadcast('ElOptionGroup', 'queryChange');
        }
        ...
        if (!val) {
          ...
          if (this.filterable) {
            ...
            if (this.multiple) {
              ...
              if (!this.remote) {
                this.broadcast('ZjTree', 'queryChange', '');
                this.broadcast('ElOptionGroup', 'queryChange');
              }
    ...
    methods: {
      ...
      getOption(value) {
        ...
        for (let i = this.nodeMap.length - 1; i >= 0; i--) {
          const cachedOption = this.nodeMap[i];
...
//        }
      ...
      handleOptionSelect(nodeMap,currentNode) {

        let value = []
        _.each(nodeMap,node=>{
          if(node.checked){
            value.push(node.label)
          }
        })
        if (this.multiple) {
          this.$emit('input', value);
        ...
        this.$nextTick(() => this.scrollToOption(nodeMap));
      },
      ...
      deleteTag(event, tag) {
        ...
        if (index > -1 && !this.disabled) {
          ...
          this.nodeMap[tag.value]['isChecked'] = false
          this.$emit('input', value);
